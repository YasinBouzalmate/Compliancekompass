...
export function Panel3({ onBack }: Panel3Props) {
  ...
  const [sortBy, setSortBy] = useState<"deadline" | "priority" | "responsible">(
    "priority"
  );
    ...
    "Må før pilot 2026": [
      {
        ...
        riskLevel: "Kritisk",
        consequence:
          "Kan ikke starte pilot uten tilgangskontroll. Potensielt brudd på GDPR og Normen.",
      ...
      },
      {
        ...
        riskLevel: "Høy",
        consequence:
          "Manglende evne til å håndtere sikkerhetshendelser. Obligatorisk for ISO 27001.",
    ...
    if (generatedActions && generatedActions.length > 0) {
      setTasks((prevTasks) => {
        ...
        const updatedTasks = CATEGORY_ORDER.reduce((acc, category) => {
          ...
          acc[category] = existingTasks.filter(
            (task) => !task.id.startsWith("generated-")
          );
          return acc;
        }, {} as TasksData);
      ...
      toast.success(
        `${generatedActions.length} tiltak generert basert på analysen!`,
        {
  ...
  const hasActiveFilters =
    searchQuery !== "" || filterPriority !== "all" || sortBy !== "priority";
  ...
  const handleExportPDF = () => {
    ...
    doc.text(`Dato: ${date} ${time}`, pageWidth - 15, yPosition + 3, {
      align: "right",
    });
    doc.text(`Bruker: CTO`, pageWidth - 15, yPosition + 7, { align: "right" });
    doc.text(`Dokument: Handlingsplan`, pageWidth - 15, yPosition + 11, {
      align: "right",
    });
    ...
    doc.text("Compliance-kompass", pageWidth / 2, yPosition, {
      align: "center",
    });
    ...
    doc.text("Prioritert handlingsplan", pageWidth / 2, yPosition, {
      align: "center",
    });
    ...
    const highPriority = Object.values(tasks)
      .flat()
      .filter((t) => t.priority === "high").length;
    doc.text(
      `Totalt ${totalTasks} tiltak (${highPriority} med høy prioritet)`,
      20,
      yPosition
    );
    ...
    Object.entries(groupedTasks).forEach(([category, categoryTasks]) => {
      ...
      doc.text(`${categoryTasks.length} tiltak`, pageWidth - 20, yPosition, {
        align: "right",
      });
    ...
    doc.text(
      "Generert av Compliance-kompass | Echomedic",
      pageWidth / 2,
      yPosition,
      { align: "center" }
    );
    doc.text(
      "Handlingsplanen oppdateres kontinuerlig ved nye krav",
      pageWidth / 2,
      yPosition + 5,
      { align: "center" }
    );
